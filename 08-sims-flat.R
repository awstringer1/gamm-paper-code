...
## Set paths ##
# basepath <- getwd()
basepath <- "~/work/projects/gamm-approximation"
...
fitnewmodel <- function(smoothformula, glmmformula, data, k = 5) {
  ...
  optfun <- function(theta) aghqmm:::optimizegammscalar(theta, yy, XX, penmat, nn, ww, control)$nll / nrow(data)
  optgrad <- function(theta) aghqmm:::optimizegammscalar(theta, yy, XX, penmat, nn, ww, control)$grad / nrow(data)
  ...
  H <- opt$hessian * nrow(data) # Scaled objfun/grad by N so rescale the hessian
  Hinv <- solve(H)
